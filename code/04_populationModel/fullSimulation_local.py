...
parser.add_argument("--home", type=str, required=True,
                    help="Project root directory (all paths are relative to this)")
...
parser.add_argument("--lambda_12_val", type=float, required=True,
                    help="Switching rate lambda_12 for this run")

args = parser.parse_args()
HOME = Path(args.home).resolve()
initial_type = args.initial
lambda_12_val = args.lambda_12_val
parameter_path = HOME / "data/processed/movementModel/fitParameters.csv"
...
# SIMULATION
def simulate(output_dir, lambda_12_val):
...
def run_simulation(lambda_12_val):
    try:
        seed = int(lambda_12_val * 1e6) + os.getpid()
        np.random.seed(seed % (2**32))
  
        output_dir = Path(HOME) /f"outputs/simulation_output/lambda_12_{lambda_12_val}"
        output_dir.mkdir(parents=True, exist_ok=True)
 
        history, B_final, lynx_final, state = simulate(output_dir, lambda_12_val)


        np.savez_compressed(
            output_dir / f"simulated_run.npz",
...
if __name__ == "__main__":
    print(f"Running simulation with lambda {lambda_12_val}.")
    run_simulation(lambda_12_val)
